...
from spectrum_preprocess import Preprocess, AtomLib  # 替换为实际的 Preprocess 所在文件名
...
class TestPreprocess(unittest.TestCase):
    def test_deisotope(self):
        ...
        expected_masses = np.array([149.5, 200.0, 200.0, 299, 300.48, 448.5, 699.5, 1399, 2098.5], dtype=np.float32)
        ...
        # 断言合并后的质量
        expected_merged_masses = np.array([149.5, 200.0, 299, 300.48, 448.5, 699.5, 1399, 2098.5], dtype=np.float32)
        ...
        # 断言最终结果
        np.testing.assert_allclose(final_masses_no_proton, expected_merged_masses, rtol=1e-5, atol=1e-5)
