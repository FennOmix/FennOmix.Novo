...
class Preprocess:
        ...
        while i < n:
            ...
            while j < n:
                if j - block_start >= peaks_per_block:
                    break
                if constrain_mz_range and (peak_mzs[j] - peak_mzs[block_start] > max_block_mz_range):
                    break
                j += 1
    ...
    def _check_other_possible_charges(
        ...
        """
        # 检查是否有足够的同位素峰且强度符合预期
        return len(isotope_idxes) >= 2 and \
               isotope_idxes[0] > 0 and \
               isotope_idxes[1] > 0 and \
               spec_intens[isotope_idxes[1]] < spec_intens[mono_idx] and \
               spec_intens[isotope_idxes[1]] < spec_intens[isotope_idxes[0]]
