...
from FeNNetNovo import PeptideMass # 后期单独处理
import numba
...
class DPDecoderTail(DecoderTail):
    ...
    def find_batch_top_k_seqs(
            self,
            batch_aa_prob_table: np.ndarray
        ...
        for i_batch in range(B):
            batch_top_k_seqs[i_batch, :], batch_top_k_scores[i_batch, :] = \
                find_top_k_seqs(top_k = self.top_k, aa_prob_table = batch_aa_prob_table[i_batch])
    ...
    def decode(
        ...
        gathered_logits = torch.gather(logits_expanded, dim=3, index=indices_expanded)  # [B, K, L, 1]
