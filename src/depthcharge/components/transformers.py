...
class PeptideDecoder(_PeptideTransformer):
    ...
    def forward(self, sequences, precursors, memory, memory_key_padding_mask):
        ...
        masses = self.mass_encoder(precursors[:, None, 0]) # masses.size: 训练 torch.Size([32, 1, 512]) precursors[:, None, 0] torch.Size([32, 1])
        charges = self.charge_encoder(precursors[:, 1].int() - 1) # torch.Size([1024, 512]) 训练 torch.Size([32, 512])
        ...
        if sequences is None:
            ...
            tgt = torch.cat([precursors, self.aa_encoder(tokens)], dim=1) #ori tokens.size:torch.Size([1024, n]),self.aa_encoder(tokens).Size([1024, n, 512]) n为已推得氨基酸个数
        tgt_key_padding_mask = tgt.sum(axis=2) == 0 #生成填充掩码
        tgt = self.pos_encoder(tgt) # 位置编码 seq == none：torch.Size([1024, 1, 512])
        tgt_mask = generate_tgt_mask(tgt.shape[1]).to(self.device) # 生成目标输入的掩码 tgt_mask，用于防止模型在解码时看到未来的信息。
        ...
        return self.final(preds), tokens #线性变换，将解码得到的512维度特征向量映射到29维（氨基酸种类）
