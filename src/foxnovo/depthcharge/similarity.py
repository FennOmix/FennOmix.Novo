...
def gsp(spectrum_x, spectrum_y, tol):
    ...
    for peak_i, (peak_mz, peak_intensity) in enumerate(spectrum_x):
        ...
        while peak_other_i < len(mz_other) - 1 and peak_mz - tol > mz_other[peak_other_i]:
            peak_other_i += 1
        ...
        while (
            ...
            peak_match_scores.append(peak_intensity * intensity_other[peak_other_window_i])
    ...
    if len(peak_match_scores) > 0:
        ...
        for peak_match_score, peak_i, peak_other_i in zip(
            ...
            if peak_i not in peaks_used and peak_other_i not in peaks_used_other:
