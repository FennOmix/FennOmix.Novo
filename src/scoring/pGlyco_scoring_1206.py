...
#
#     return result_df
def filter_mass_match(df, mass_tol, mass_error, sequence_mass_col="modified_sequence mass", precursor_mass_col="precursor_mass"):
    """
    根据mass_tol（Da偏移）和mass_error（ppm误差）过滤DataFrame，保留满足质量匹配的行
...
class DenovoSequenceScoring:
    def __init__(self, sequences_df, hdf_path):
        ...
        try:
            self.spectra_df = f.ms_data.spectrum_df.values[
                ['charge', 'peak_start_idx', 'peak_stop_idx', 'precursor_mz', 'spec_idx',
                 'ms_level']]  # 此处为ms.data 可能也会是raw.data
        except:
            self.spectra_df = f.psm.psm_df.values[['charge', 'peak_start_idx', 'peak_stop_idx', 'precursor_mz', 'spec_idx']]
        ...
        self.sequences_df['top'] = self.sequences_df['sequence_rank'].str.extract(r'(\d+)').astype(int) # 用于记录模型原始输出排名
    ...
    def calculate_peptide_mw(self, peptide: str) -> float:
        ...
        while pos < peptide_len:
            ...
            if not matched:
                ...
                raise ValueError(f"肽段 '{peptide}' 中存在未定义的氨基酸：{peptide[pos:pos + 3]}...")

        # 加水电分子量，返回结果
        return total_mass + self.WATER_MW
    def add_mw_to_df(self, df: pd.DataFrame, peptide_col: str = "肽段序列", mw_col: str = "肽段分子量"):
        ...
        return df
    def calculate_precursor_mass(self, df:pd.DataFrame, precursor_mz_col: str = "precursor_mz", charge_col: str = 'charge'):
        df['precursor_mass'] = (df[precursor_mz_col] * df[charge_col] - df[charge_col] * 1.00728).round(2).astype(np.float64)
        return df
    "------分子量计算------"
    ...
    def sequence_spectra_match(self):
        self.sequences_df = pd.merge(
            self.sequences_df,
            self.spectra_df[['charge', 'peak_start_idx', 'peak_stop_idx', 'precursor_mz', 'spec_idx', 'ms_level']],
            on='spec_idx',
            how='inner'
        ...
        fragment_mz_df = create_fragment_mz_dataframe(psm_df, charged_frag_types) #已核对：随便计算无误
        ...
        psm_df = self.add_mw_to_df(psm_df,peptide_col='modified_sequence',mw_col='modified_sequence mass')
        psm_df = self.calculate_precursor_mass(psm_df,precursor_mz_col='precursor_mz', charge_col='charge')
        # 提取并转换谱图数据数组
        ...
        self.matched_mz_err_ppm_df = pd.DataFrame(
            (matched_mz_errs / all_frag_mzs) * 1e6,
            columns=fragment_mz_df.columns
        )
    ...
    def filter_score_and_mass(self):
        ...
        self.filtered_sequences_df = self.filtered_sequences_df[self.filtered_sequences_df["score"] >= 0.00001]
        ...
        drop_cols = ['sequence_rank', 'top', 'peak_start_idx', 'peak_stop_idx', 'ms_level', 'row_idx', 'frag_start_idx',
                     'frag_stop_idx']
