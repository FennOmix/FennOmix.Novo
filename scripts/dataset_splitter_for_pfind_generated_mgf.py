...
# 但需要注意：1. 本代码中的修饰、标准氨基酸控制需加入mgf+search_result >  hdf的脚本中
...
def build_mod_seq(sequences, mods_list, sites_list, mod_dict):  # noqa: C901
    """高性能构建带修饰的肽段序列"""
    result = []
...
def filter_and_remap_peaks(spectrum_df, peak_df):
    ...
    for s, e in zip(start_indices, stop_indices, strict=False):
...
class HDFDataSplitter:
    ...
    def _preload_sequences(self):
        ...
        for file_idx, path in enumerate(self.hdf5_paths):
            ...
            psm_df = f.psm.psm_df.values[
                ["sequence", "mods", "mod_sites", "precursor_mz", "charge", "spec_idx"]
            ]
            psm_df = psm_df[
                (psm_df["sequence"].str.len() >= 8) & (psm_df["sequence"].str.len() <= 14)
            ]

            psm_df = psm_df[
                psm_df["sequence"].apply(self._is_standard_sequence)
            ].copy()  # 要求氨基酸为标准氨基酸
            ...
            for orig_idx, row in psm_df.iterrows():
                ...
                sequence_info[sequence].append(
                    (file_idx, orig_idx, row["precursor_mz"], row["charge"], row["spec_idx"])
                )
        ...
        if n_test > 0:
            ...
            for seq, pairs in self.test_sequence_info.items():
                for file_idx, orig_idx, prec_mz, charge, spec_idx in pairs:
                    self.test_file_groups[file_idx].append(
                        (orig_idx, seq, prec_mz, charge, spec_idx)
                    )
    ...
    def hdf_generator(self, file_groups, output_folder):
        ...
        for key, _ms2_list in tqdm(file_groups.items()):  # 逐HDF读取
            current_handle = HDF_File(file_name=self.hdf5_paths[key], read_only=True)

            # 解包所有信息（保留你的原有解包）
            idxs, seqs, prec_mzs, charges, spec_idxs = zip(*file_groups[key], strict=False)
            ...
            if not any(valid_psm_mask):
                print(
                    f"文件 {self.hdf5_paths[key]} 无有效数据（spec_idx+charge+mz协同匹配失败），跳过"
                )
                continue
            ...
            # 同步过滤其他列表
            prec_mzs = [mz for mz, valid in zip(prec_mzs, valid_psm_mask, strict=False) if valid]
            charges = [c for c, valid in zip(charges, valid_psm_mask, strict=False) if valid]
            spec_idxs = [
                idx for idx, valid in zip(spec_idxs, valid_psm_mask, strict=False) if valid
            ]
            seqs = [seq for seq, valid in zip(seqs, valid_psm_mask, strict=False) if valid]
            ...
            seq_psm_df = seq_psm_df.drop(
                columns=["peak_start_idx", "peak_stop_idx"], errors="ignore"
            )
            ...
            output_hdf = HDF_File(
                file_name=output_hdf_path, read_only=False, truncate=True, delete_existing=True
            )
            output_hdf.psm = {}
            output_hdf.ms_data = {}
